...
A data analytics portfolio project built on top of [dbt Labs' Jaffle Shop](https://github.com/dbt-labs/jaffle-shop) sandbox (a fictional restaurant chain), extended with a full dbt analytics layer, business SQL analyses, and an interactive Streamlit dashboard — end to end, from raw seed data to insight.

**Dataset:** 935 customers, 61,948 orders, 6 store locations, 10 products (synthetic/fictional data generated by the original template).

## What's original here vs. the base template

The base Jaffle Shop template provides the raw seed data, a minimal staging layer, and dbt Cloud setup docs (kept below for reference). Everything past that is custom:
...
- Revenue grew 5.8x over the observed period ($16K → $94K/month)
- 77% of orders are drink-only (AOV $6.36) vs. combo orders averaging $27 (4x)
- Only 2 of 6 locations are active in the current data (Philadelphia + Brooklyn); Philadelphia outperforms Brooklyn ~2x on monthly profit
- "Cannot Lose Them" segment: 82 high-value customers (avg spend $998) with no recent orders — a churn-risk group worth targeting
- Margin/volume tradeoffs surfaced per product (e.g. one item at 89% margin but low volume — a growth opportunity; another at 29% perishable-waste risk)
- Churned customers skew more food-heavy (33% vs. 18% for retained) with lower average order value — drink-habit customers are the sticky base
- 90-day LTV forecast (BG/NBD + Gamma-Gamma): $273,746 total across 929 repeat customers

### Two methodology bugs caught and fixed during the churn model (worth reading if you're building similar models)
1. **A broken label.** The initial churn definition ("no order in 90 days," measured against the dataset's own last date) produced 1 churned customer out of 935 — this customer base orders every 3-6 days, and the dataset simply ends while the business is still fully active, so almost nobody has time to go quiet before the data runs out. Fixed by holding back a genuine 30-day window and checking real future behavior instead, which yields a usable 18% churn rate. See `models/marts/analytics/fct_customer_churn_features.sql`.
2. **A tautological feature set.** With that fixed, a first model using recency-window features (`orders_last_30_days`, `days_since_last_order`, etc.) scored ~100% ROC-AUC — too good to trust. It turned out `orders_last_30_days == 0` predicts the churn label correctly 97% of the time on its own, since customers who go quiet in this dataset essentially never come back. That's the label restated, not an early-warning signal, so those features were dropped in favor of lifetime/behavioral ones. The resulting model (~99% AUC) is still unusually clean — most likely because this is synthetic data built from a handful of consistent customer archetypes, cleaner than real purchase behavior. Full writeup in `python/04_churn_model.py`.

## Status

- [x] dbt analytics layer
- [x] SQL business analyses
- [x] Streamlit dashboard
- [x] Predictive models (churn + LTV + anomaly detection)
- [ ] Additional export/reporting integrations (Google Sheets, Power BI, Notion)

## Running it locally

```bash
pip install -r requirements.txt
dbt seed        # loads the raw CSVs in seeds/ into DuckDB
dbt run         # builds all models, including the analytics layer
dbt test        # runs the test suite
streamlit run dashboard/app.py
```

`jaffle_shop.duckdb` and `exports/` are generated by the commands above and are gitignored — nothing sensitive, just build output.

To run the predictive models as standalone terminal reports (same code the dashboard's Predictions page uses):
```bash
